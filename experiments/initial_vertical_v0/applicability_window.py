...
즉 이 도구는 0.1 s 표본 하나를 0.1 s 로 누적하므로 중간 49 스텝을
보지 못한다. 놓치는 것은 중간 **실패**만이 아니다 — 중간 **성공**도
놓치므로 결과를 일반적으로 상한이라고 부를 수 없다. 방향은 사례마다
다를 수 있다.

**확정값은 하니스가 낸다.** 하니스 루프가 매 적분 스텝을 직접 집계하고
그 결과를 [APPLICABILITY] 줄로 내며 run_cases.sh 가 값으로 단언한다.
이 스크립트는 그 확정값과 대조하거나 다른 CSV 를 살펴보는 용도다.
"""
import argparse
import csv
import math
