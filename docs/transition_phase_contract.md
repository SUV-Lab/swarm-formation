# TransitionPhase 설계 계약 v4 (계약 2)

- 상태: **설계 확정 — v1 권위 모델은 ADR-0002** (2026-08-07). 실제 플랫폼
  인증이 아니라 연구·구조 검증이 목표이며, 외부 스펙은 기다리지 않는다.
  - **v1 권위 모델**: 프로젝트 정의 중립 3DOF 점질량 벤치마크(공개
    운동방정식 + 명시적 가정 파라미터 — `docs/adr/0002-neutral-benchmark.md`,
    구현 `mmp_vehicle_dynamics/point_mass_eom.hpp`)
  - **외부 6DOF**(experiments/jsbsim_probe): 독립 민감도 비교 하니스 —
    합격 판정자가 아니다
  - **실제 모델 데이터 도입**: 향후 별도 ADR
  - 같은 공식·같은 파라미터 간 격자 비교는 **consistency test**라
    부른다(물리 교차검증 아님) — `docs/transition_v1_test_design.md`
  - §13 구현은 ADR-0002·테스트 설계 리뷰 통과 후 시작. 모든 산출물 표기:
    "프로젝트 정의 중립 3DOF 벤치마크 기반 구조 검증"
- 채택 근거: 실제 요구가 "순항 영역 밖 초기 상태에서 시작해 순항 궤적으로
  연결"로 확정됨(2026-08-06). 초기 벡터를 영역 안으로 바꾸는 것은 계약 1
  데모일 뿐 요구 해결이 아니다.
- **필요한 것은 두 종류다**: 한계 수치(§8)는 "무엇을 허용하는가"만 말하고
  상태를 시간 전개하지 못한다. 전개할 **동역학 모델**(§11)이 함께 있어야
  전이 생성기가 성립한다.
- 전제(계약 1, 구현 완료): 명시 초기 상태의 **PVA 전체** 판정
  (`PathManager::pvaEnvelopeProblem` — 속력·상승각 영역 + 역동역학
  하중·추력·뱅크, 판정 불가 시 거부) → `INITIAL_MODE_UNSUPPORTED` FAILED;
  phase 모드 direct fallback의 비행 적합성 게이트(fail-closed: 평가 불가도
  FAILED) → `DIRECT_FALLBACK_UNSAFE`; 유한성 검사는 모델 off 조기 반환보다
  **앞**. 회귀 31종 전부 통과 (initfail / initaccfail / initnan / initok /
  synthclamp / unsafedirect / departop 포함).
- 계약 1 라이브 마감 (2026-08-06, full_map r5 회랑):
  - 콘 안 정상 상태 (189.5 m/s, 상승각 7.3°, 가속도 0): 계획 성공 —
    departure @s=32.8 u, arrival @s=2203.8 u, 205조각 1111.3초, 판정 CLEAN,
    총 1057 ms. 로그 `replan_fsm_drone_0_20260806_081440.log`.
  - 콘 밖 (상승각 32°): 명령 수신과 **같은 밀리초**에
    `INITIAL_MODE_UNSUPPORTED` 거부, FM·옵티마이저 미가동. 로그
    `replan_fsm_drone_0_20260806_083349.log:73`.
  - 부수 확인: r5 원본의 초기 가속도 `[3, 0, 2] m/s²`는 161 m/s에서
    최대 추력의 116%를 요구한다 — 상승각을 콘 안으로 낮춰도 PVA 판정이
    거부한다. z만 바꾸는 것으로는 부족하다는 실측 근거.

## 1. 역할 정의 — 두 "출발"의 의미 분리

- TransitionPhase: 순항 모델 **밖**의 초기 PVA를 유효 영역 **안**으로
  데려오는 구간. 전용 전환 모델·한계 집합으로 계획하고 검사한다.
- 기존 departure(체인 1구간): 유효 영역 **안**의 상태를 전역 경로에
  정렬하는 구간. 순항 모델 그대로.
- 겹치지 않는다. TransitionPhase의 종료 상태가 곧 departure가 시작할 수
  있는 상태다.
- 근거 결함: 상승각 32°(콘 ±30°) 초기 벡터가 계약 1 이전에는 직행 강등,
  그 이전에는 548.7 u 벌레 궤적 SUCCESS
  (logs/runtime/replan_fsm_drone_0_20260806_054645.log, 피크 추력 299.2%는
  054808 · 064053 로그).

## 2. 실행 분기 계약 — 시작 상태 분류

계획 진입 시 명시 초기 PVA를 세 모드 중 하나로 분류한다. 문자열 반환이
아니라 열거형 판정이어야 한다(현행 `pvaEnvelopeProblem`의 문자열은 계약 1
의 통과/거부 이분법용 — 계약 2 구현 시 분류기로 대체).

```
CRUISE_VALID          순항 영역 안
  → TransitionPhase 생략, 기존 체인 그대로 실행
TRANSITION_REQUIRED   순항 영역 밖 + 전환 영역 안
  → 전역 경로 → TransitionPhase → 경로 슬라이스 → 기존 체인
UNSUPPORTED           전환 영역까지도 밖
  → 즉시 FAILED(INITIAL_MODE_UNSUPPORTED)
```

- 가운데 분기는 **구현 완료** (2026-08-08, `classifyStartState` +
  `planTransitionMission`): `transition/enable`이 열어 준다.
- **기본값은 `true`다** (`optimizer_params.yaml`, 커밋 `37fc51a` "transition ships
  on: planning the launch phase is the point of the stack"). 이 문서는 한동안
  "기본 false"라고 적혀 있었는데 출하 설정과 어긋난 상태였다 — 설정이 옳고 문서가
  낡았다.
- 끄면(`transition/enable=false`, 세션 단위로는 `launch에 transition:=0`) 계약 1의
  원래 결과가 그대로 유지된다: `INITIAL_MODE_UNSUPPORTED`, 사유에 "(transition
  disabled)" 접미. 회귀 `initfail`/`initaccfail`/`initceiling`/`pvaprobe`가 그
  꺼진 동작을 NodeOptions로 플래그를 내려 고정한다 — 출하 기본값이 무엇이든
  양쪽 동작이 각각 자기 변형으로 고정되어 있다.

## 3. 파이프라인 (수렴 구조)

```
[실제 초기 PVA]
   ↓ 분류기: TRANSITION_REQUIRED     (전역 FM 경로는 이미 존재)
전이 생성기 — 전환 모델·한계로 시간 궤적 PVA(t) 생성
   목표 = 전역 경로 위 진입점 (§5)
   ↓
다항 어댑터 — Hermite/MINCO 구간 구성 (§6 내부 검사 필수)
   ↓
[2단계, 후속] 참조 추종 L-BFGS — §7 계약
   ↓
단계별 평가기 (§4) — 전환 구간은 전환 모델로
   ↓ 종료 PVA = prescribed head
SegmentChainPlanner — 경로 슬라이스 [route_start_s .. goal] 재계획
```

- 구현 순서 확정: **1단계 = 최적화 없는** 전이 생성기 → 다항화 → 검증 →
  C² 체인 연결. L-BFGS 참조 추종은 별도 2단계 비교 실험으로 추가.

## 4. 단계별 평가기 — "공용 검사"라는 말을 버린다

한 평가기가 전 구간을 검사할 수 없다. 단계마다 모델이 다르다:

| 대상 | 판정 모델 |
|---|---|
| TransitionPhase 구간 | 전환 모델·전환 한계 (§8) |
| departure/cruise/arrival | 순항 모델 (기존 per-solve 감사 그대로) |
| 접합부 (전이→체인, 체인 내부) | PVA 오차 게이트 + jerk 점프 측정 (§6·§8c) |
| 전체 결합 궤적 | 지형·구역·시간·수치 건전성 |

- **로깅과 판정 분리**: 현행 `logFinalEvaluation`은 로그+판정 겸용이고,
  스티치된 체인은 그 판정을 버린다(정보성). 계약 2에서는 판정 전용
  `evaluateFlight()`를 분리해 위 표의 네 판정을 모두 반환하고, 로그는
  그 결과를 출력만 한다. direct fallback 게이트도 이것을 쓰도록 이관.
- 모든 판정은 fail-closed: 평가 불가 = 부적합.

## 5. 접속 계약 — 경로 슬라이스와 prescribed head

- 공간 정합은 "경로 우선": 전역 FM 경로를 먼저 만들고 전이 생성기가 그
  경로 위 진입점을 목표로 삼는다. 경로는 전역 1회 결정(r3 교훈). 투영·
  잔여 경로 재생성 방식은 채택하지 않는다.
- 진입점 좌표는 **호장 `route_start_s`** 로 확정한다(체인 접합이 호장
  기반이므로 일관, 진입점이 기존 vertex 위일 필요도 없다). vertex index는
  파생값으로만 쓴다.
- 체인 입력은 전역 경로 전체가 아니라 진입점부터 잘린 슬라이스다:

```
전역 경로  [start ...... s=route_start_s ...... goal]
                          ↑ TransitionPhase 종료
체인 입력  = 경로[route_start_s .. goal]
prescribed head = TransitionPhase 종료 PVA
```

- 슬라이스 없이 전체 경로를 넘기면 체인이 원래 시작점부터 departure를
  다시 만들어 전이 구간과 중복된다 — 금지.
- 중기 궤적은 "있는 것에 붙이는" 게 아니라 전이 종료 PVA를 head로 받아
  그 시점에 다시 푼다.

### 요청/결과 타입 (구현 시 이 형태로)

```
TransitionRequest  { initial_pva, global_route(+cap), entry_candidates[]
                     (기존 선별 규칙 재사용), transition_limits,
                     zone_policy[] }
TransitionResult   { verdict(OK/FAILED+사유), transition_traj(PVA(t)),
                     end_pva, route_start_s, audit(전환 평가기 결과) }
```

- `zone_policy[]`: 전역 3-pass가 확정한 **구역별** 조성 스냅샷 —
  **구현 완료**: `PathManager::zonePolicySnapshot()`. 패스 번호는 정책이
  아니다: 조성은 4단으로 구분된다(`HARD_AVOID` = 접촉 시 후보 실격 /
  `SOFT_UNAVOIDABLE` = 전역 경로가 실제 필요로 한 횡단 / `SOFT_ENDPOINT`
  = 시작·목표 포함 면제 / `SOFT_FALLBACK` = 패스 2 전면 소프트 필드).
  - 스냅샷은 인덱스가 아니라 **형상 사본 + 세대(데이터)·에포크(검색)
    ID**를 갖는다. 유효 조건은 "현재 데이터 위에서 **단일 목표 검색이
    정확히 1회**" — 다중 경유점 미션은 leg마다 정책 상태를 덮어써
    마지막 leg만 남으므로 INVALID로 fail-closed 한다(회귀
    zonemultileg).
  - 접촉 판정은 공용 `zoneContact()`로만 한다 — 전역 검색이 경로를
    배제하는 **하드 배제 볼륨 그대로**(1.05x 타원체 + 가시성 > 0.35
    스탠드오프; 공칭 1.0x/0.5가 아니다 — 가시성 (0.35, 0.5] 밴드는
    위험이 양수다)이며 면제 필터가 없다(면제는 조성이 정책으로 든다).
    결과는 4상
    `CLEAR/CONTACT/STALE/INVALID`: STALE(존/지형 변경 또는 이후 검색)과
    INVALID(3-pass 미실행·다중 leg)는 "접촉 없음"으로 못 읽는다 —
    전이는 실패하거나 경로부터 재커밋한다(회귀 zonesnapshot·zonepass0).
  - 연속 위험 노출은 판정이 아니라 측정이며 별도 API
    `zoneExposureRaw()`로 얻는다 — RAW = 무테이퍼 원시 필드(최적화기의
    비용 필드가 아니다; 후보 비용에 쓰려면 테이퍼 일치 변형을 그 비용
    설계와 함께 추가). 생성기의 기하 재구현 금지.

## 6. 다항 어댑터 — 구간 내부 검사와 접합 감사

- Hermite 변환은 근사다. 샘플 점 일치가 샘플 사이 준수를 보장하지 않는다.
- 고정 간격 샘플링만으로 끝내면 안 된다. 둘 중 하나 이상:
  - 다항 극값 검사: 검사량(속력², 상승률 등)을 미분해 구간 내 극값을 직접
    판정.
  - 적응형 세분화: 위반 여유가 얇은 구간을 재귀 분할, 수렴까지.
- 접합 연속성: 체인은 C² (P/V/A ~1e-13). C³은 보장하지 않는다. 전이→체인
  접합의 jerk는 "연속 보장"이 아니라 **좌/우 극한 jerk 차 노름의 점프
  감사**로 정의한다 — §8(c)에 따라 측정 전용이며, 상한 판정은 실행
  계층의 실제 허용치가 생겨 게이트로 승격될 때만 붙는다.

## 7. 참조 추종 최적화 계약 (2단계, 후속)

- 시작·종료 PVA: hard (경계 조건, 이동 금지).
- 전이 원형(생성기의 PVA(t)): reference 비용 — L-BFGS가 전이 알고리즘의
  의도를 지우지 못하게.
- 전환 한계 집합: gate — 순항 한계가 아니라 전환 전용 수치로 판정.
- 시간 배분: 제한적 조정만 허용 (원형의 시간 구조 보존).
- 1단계(무최적화)와의 품질 비교 실험으로 채택 여부를 결정한다.

## 8. 전환 한계 수치 — 확정 (2026-08-08, 3분류)

수치는 성격에 따라 세 부류로 나뉘고, 부류가 취급을 결정한다.

**(a) 모델 하드 한계 — 주입된 Parameters에서 계산, 리터럴 금지.**
게이트로 강제되며 경계 3점(아래/경계/위) 회귀로 고정
(s8limits·s8bounds; 경계 의미론은 ±ε 포함 — 곡률 경로에서 동적 양의
정확-등호 유지는 구성 불가하므로 포함측 증인은 표류 안쪽 점).

- 전환 속력 구간: [model_activation_speed, speed_max] — 분류기 대역과
  스텝별 한계 게이트가 같은 값.
- 상승각(구조): 모델 구조 콘 |γ| < acos(kMinCosGamma) ≈ 1.521 rad —
  특이점 경계. 정책 콘 1.40 rad는 아래 (b)의
  `max_abs_gamma_rad`로만 존재하며 게이트로 강제된다(중복 정의 금지).
  순항 콘 ±30°는 순항 판정 전용 — 전이 중에는 적용하지 않는다.
- CL·추력 범위·하중계수·동압·뱅크: pointMassForces 폐쇄와 검증기·스텝
  게이트가 동일 Parameters로 강제.
- 인계 속력 상한: **유효 상한 = min(모델 speed_max, 명시 캡, 계획
  캡)** — `effectiveHandoffMaxMps()` 한 곳에서 정의, 분류기·생성기·종료
  판정 공용.

**(b) 계획 정책 — 선택값 (근거는 코드 주석·구현 노트).**
전이 정책 상승각 콘 `TransitionLimits::max_abs_gamma_rad` = 1.40 rad
(한 정의 — 분류기·스텝 프리가드·다항 검증기가 같은 값을 읽고, 구조 콘
안에 있어야 생성이 수락된다; 정책이지만 게이트로 강제),
포획 횡회랑 10 m·접선 정렬 0.10 rad·dwell 1.0 s(§9 체류 조건),
t_go 창 [1.5 s, 0.85×blend_t_max], 조준 선행 4 s, LOS 크로스페이드
5 s, 추력 서보 400 N/(m/s), idle 마진 2%, 종료 하한 = speed_min×(1+
margin)(§9 겹침 하한과 동일). 변경은 결정성·실현성 회귀 재통과 필요.

**(c) 관측 통계 — 측정 전용, 합격 게이트 아님.**
접합 jerk 점프·시작 가속 불일치/모델값·어댑터 적합 오차·노출 통계.
특히 jerk 상한은 생성 결과 분포로 역산하지 않는다(순환 논리 기각) —
추종기/실행 계층의 실제 허용치가 생겼을 때만 게이트로 승격한다.

## 9. 순항 진입 조건 — 모델 겹침 영역과 margin 배치

현재 코드에 섞여 있는 경계를 역할별로 확정한다:

| 값 | 역할 |
|---|---|
| 122 m/s (모델 speed_min) | 순항 모델의 원(raw) 하한 — 감사·평가 기준 |
| 132 m/s (margin 포함 floor) | **시작 상태 경계** — 계약 1 검증과 합성 시작 클램프가 쓰는 값. 전이 **종료 상태도 이 값 이상**이어야 한다 (체인 head가 되는 순간 같은 검증을 받으므로) |
| 230 m/s (모델 speed_max) | 상한 — margin 없음, 시작·감사 동일 |
| ±30° (비행경로각) | 콘 — margin 없음, 시작·감사 동일 |

- 전이 완료는 경계에 "걸치는" 것이 아니다: **순항 모델과 전환 모델이
  동시에 유효한 겹침 영역**에 들어와 §8의 체류 조건(속력 수렴 폭·접선
  정렬·체류 시간)을 채워야 완료다. 겹침 영역의 하한은 위 표의 margin
  포함 floor를 쓴다 — 전이가 122 m/s 언저리에서 끝나면 체인 head 검증이
  즉시 거부하는 모순을 막는다.

## 10. 실패 의미론과 테스트 계약

실패 사유(구현 시 PlanReason 확장 또는 TransitionResult.verdict):

- `INITIAL_MODE_UNSUPPORTED` — 분류기 UNSUPPORTED (전환 영역까지도 밖)
- `TRANSITION_GENERATION_FAILED` — 전이 생성기가 진입점까지 못 감
- `TRANSITION_ADAPTER_UNSOUND` — 다항화 내부 검사 실패 (극값/세분화)
- `TRANSITION_JUNCTION_UNSOUND` — 접합 PVA 오차 (jerk 점프는 §8c 측정
  전용 — 게이트 승격 전에는 이 사유를 만들지 않는다)
- 전환 영역에서 direct fallback은 없다 — 순항 플래너는 그 영역을 못 푼다.
  위 사유는 전부 미션 FAILED다.
- 전이 성공 + 체인 강등 → 체인의 기존 DEGRADED 의미론 그대로.

회귀 계약 (구현 시 하니스 변형으로 추가):

1. CRUISE_VALID 입력 → TransitionPhase 생략, 기존 체인과 결과 동일
2. TRANSITION_REQUIRED 입력(예: 상승각 32°) → 전이+슬라이스 체인, 접합
   C² + jerk 점프 측정(§8c), 전이 구간은 전환 한계로만 판정
3. UNSUPPORTED 입력 → 즉시 FAILED, FM/옵티마이저 미호출 (injection 트립와이어)
4. 슬라이스 검증: 체인 시작 = route_start_s, 전이 구간과 경로 중복 없음
5. 전이 종료 상태가 겹침 영역 밖(<132 m/s 등)이면 접속 거부

## 11. 전환 동역학 모델의 출처 (코드 조사로 확정, 2026-08-06)

### 11.1 결정적 사실 — 순전파가 저장소에 없다

- `mmp_vehicle_dynamics`는 **역동역학 전용 라이브러리**다. 공개 함수 7개
  (`parametersAreValid`, `airDensity`, `evaluateInverseDynamics`,
  `flightEnvelopePenalty`, `envelopeUtilization`, `isWithinEnvelope`,
  `envelopeLimitName`) 중 **시간을 진행시키는 것은 하나도 없다**.
  "PVA를 주면 필요한 힘"만 답한다.
- 유일한 RK4 순전파(`dynamics_sim_node.cpp:359 integrateRk4`)는 노드
  클래스의 private 멤버이며 실행 파일에만 컴파일된다. 설치 헤더에도,
  Python 바인딩에도 없다. 게다가 `derivative()` → `controller()` →
  `sampleReference()` 구조라 **기존 PolyTraj가 있어야 도는 추종기**다 —
  생성기로 쓸 수 없다.
- ⇒ **전이 생성기의 순전파는 새로 만들어야 한다.** "적분기를 재사용하면
  된다"는 전제는 사실이 아니다. (3DOF 운동방정식 자체는
  `dynamics_sim_node.cpp:331-342`가 올바르므로 들어낼 수 있으나, 그 주위의
  컨트롤러·클램프·준수직 가드는 못 쓴다.)

### 11.2 결론: 별도 `TransitionDynamics` **인터페이스**가 필요하다

확정 수준을 구분한다 — 코드 조사가 증명할 수 있는 것은 "현재 모델로는
안 된다"까지이고, "새 모델에 어떤 물리가 들어가야 하는가"는 플랫폼
요구사항이 정한다.

| 항목 | 확정 수준 |
|---|---|
| 별도 `TransitionDynamics` 인터페이스 필요 | **확정** (아래 장벽으로 증명) |
| 그 뒤를 자체 구현할지 공개 모델을 붙일지 | **ADR-0002 중립 3DOF 벤치마크 채택**; 외부 모델은 민감도 비교 전용 (§11.5 이력) |
| 상태·제어·물리 채널 구성 | **ADR-0002(중립 3DOF 벤치마크)로 확정** — 공개 EOM + 명시적 가정 파라미터 |
| 질량 변화, 마하 의존 항력, 추가 제어 채널(추가 구동 채널 등) | v1 벤치마크 범위 밖 — 점질량 모델의 선언된 비포함 항목 (ADR-0002 참조) |

기존 `Parameters` 확장으로 못 넘는 구조적 장벽(코드로 증명된 부분):

| 장벽 | 근거 |
|---|---|
| `speed_min_mps`가 이중 역할 | 실속 한계이자 q-floor 정규화항 (`flight_dynamics.cpp:169`). 전환용으로 낮추면 CL·유도항력의 1/\|v\|² 특이점 보호가 사라진다 (측정: V=60에서 CL 1.21 vs 참값 5.02, 속도 그래디언트 노름 9.6e2 → 4.1e4) |
| 준수직 표현 불가 | `parametersAreValid`가 `flight_path_angle_max_rad ≥ 90°`를 하드 거부 (`:362`, 실행 확인). 어떤 파라미터 조합으로도 수직 발진을 표현할 수 없다 |
| 법선력 채널이 날개 양력뿐 | `:185-187`이 법선 비추력 전체를 날개 CL로, `:225`가 추력을 속도 접선으로 배정. 이 구조 자체가 고정이라, 다른 법선력 채널이 필요한 플랫폼이면 파라미터로는 표현할 수 없다 |
| 질량 일정 | 저장소 전체에 mdot/Isp/추진제 없음 (구조적 부재). 전환 구간에서 질량이 유의미하게 변하는 플랫폼인지는 **스펙이 답할 문제** |

정량 근거: 순항 속도에서 **상승각 32°·가속도 0**만으로도 이미 7947 N =
`thrust_max`의 2.48배를 요구한다. 순항 파라미터 집합은 잘못 튜닝된 게
아니라 **다른 비행 모드를 기술하고 있다**.

단, 통째로 분기하지는 않는다:

- `Parameters`(대기·항력극선·형상)는 공용 기반으로 유지.
- `TransitionDynamics`는 그 상위집합. 추가 항목의 **목록은 스펙이 정한다**.
- **겹침 영역 축약 시험 필수**: 속력 ≥ speed_min, |γ| < 30°, T < thrust_max
  영역에서 기존 `evaluateInverseDynamics`와 일치할 것. 서로 다른 구현이므로
  비트 단위 일치는 요구하지 않는다 — 요구 사항은 셋:
  1. PVA·힘·한계 사용률이 물리량별로 정의된 **허용오차 이내**
  2. `isWithinEnvelope` **판정이 동일**
  3. 경계 근처는 별도 tolerance/hysteresis 적용 (한계 바로 위아래에서
     두 구현이 반대 판정을 내는 것을 시험 실패로 삼지 않는다)

  이 시험이 "모델 권위는 한 곳"이라는 원칙을 지킨다.

### 11.3 검증 기준 — 수치 검증과 물리 검증은 다른 것이다

오늘 물리 검증에 쓸 수 있는 것이 없다:

- 역동역학 모델의 플로어 이하 CL·항력은 허구(측정 4.13배·3.45배 과소).
- `isWithinEnvelope`/`envelopeUtilization`은 정의상 순항 영역을 인코딩.
- `dynamics_sim_node`의 RK4는 순항 한계로 포화된 추종기.

검증은 세 층으로 나누고, 각 층이 무엇을 보증하는지 혼동하지 않는다:

| 층 | 수단 | 보증하는 것 | 보증하지 못하는 것 |
|---|---|---|---|
| 수치 자기 일관성 | 전환 EOM의 독립 RK4 순전파 vs 생성 궤적의 **잔차** | 적분·다항화 구현이 자기 모순 없음 | **모델이 현실과 맞는지** |
| 물리 타당성 | 권위 있는 모델·시험 데이터·외부 시뮬레이터 | 전환 모델이 실제 기체를 기술함 | — (외부 입력 필요, 현재 없음) |
| 인계 적합성 | `PathManager::pvaEnvelopeProblem` 그대로 재사용 | 순항 체인이 그 상태를 날 수 있음 | 전이 구간 내부의 타당성 |

- 인계 판정은 오늘 `INITIAL_MODE_UNSUPPORTED`를 올리는 바로 그 게이트다.
  전이 종료 상태에서 이 함수가 빈 문자열을 반환하는 것이 인계 성공의 정의.
- **물리 타당성 층이 비어 있는 채로 계약 2를 구현하면, 자기 일관성만 있고
  현실성은 아무도 보증하지 않는 전이 궤적이 나온다.** 이것이 스펙 확보가
  구현보다 앞서야 하는 이유다.

### 11.5 순전파 모델 — 결정 이력 (ADR-0002로 종결)

초기 검토는 공개 6DOF 모델 통합을 우선했으나, ADR-0001 기각과 재선정
조사를 거쳐 **v1 권위 모델은 프로젝트 정의 중립 3DOF 벤치마크**(ADR-0002)
로 확정됐다 — 체인이 소비하는 것은 PVA·C²뿐이고, 전환·순항이 같은 공식·
같은 파라미터를 쓸 때만 "모델 권위는 한 곳" 원칙이 구성적으로 성립하기
때문이다. 아래 표는 그 검토의 기록이며, 외부 모델의 현재 역할은 **독립
민감도 비교**로 한정된다.

| 후보 | 성격 | 이 저장소에서의 위치 |
|---|---|---|
| JSBSim | C++ 범용 비선형 6DOF 비행 동역학, 공력·추진·제어를 데이터 파일로 구성 | **1순위 통합 후보** — C++ 코드베이스와 직접 연결 |
| RocketPy | Python 3/6DOF, 가변 질량·모터·대기 모델 포함 | **오프라인 참조·비교 검증** — 실시간 C++ 플래너에 직접 넣기는 부적합 |

**모델이 주는 것과 주지 않는 것을 혼동하지 않는다:**

- 동역학 모델: 입력 u(t)를 주면 상태 PVA(t)를 계산 — **가져올 수 있다**
- 전이 플래너: *어떤* u(t)를 줘야 순항 진입점에 도달하는가 — **직접 구현**
- MINCO 연결: 결과 PVA(t)를 다항식으로 표현하고 C²로 접합 — **직접 구현**

채택 절차:

1. JSBSim을 외부 참조 순전파 모델로 선정, C++ 연동·PVA 추출 가능성만 확인
   (제공 예제 기체로 구조 데모)
2. 실제 플랫폼 파라미터를 모델 파일에 입력
3. 전이 입력 프로파일 후보를 순전파해 실행 가능 궤적 생성
4. 출력 PVA(t)를 Hermite/MINCO로 표현 → 기존 체인의 prescribed head에 C² 연결
5. 참조 추종 L-BFGS는 그 뒤 필요할 때만

**주의**: 처음부터 JSBSim을 L-BFGS 내부에 넣지 않는다 — 무겁고 미분 구조가
현 최적화기와 맞지 않는다. 먼저 "후보 생성·검증용 순전파 모델"로 쓴다.

**충실도**: 후단 계약이 PVA와 C² 연결 중심이므로 3DOF(또는 단순화 병진
모델)로 기능을 먼저 증명한다. 자세·각속도·구동기 응답이 전이 가능성을
좌우한다는 요구가 확인되면 6DOF로 확장.

### 11.6 JSBSim 연동 타당성 측정 결과 (2026-08-07, 2단계 완료)

컨테이너에서 JSBSim v1.2.1을 소스 빌드하고 C++ 프로브로 직접 측정했다.
프로브는 `logs/jsbsim_probe.cpp` (gitignore).

| 질문 | 결과 |
|---|---|
| ROS2 Humble 컨테이너에서 빌드·링크되는가 | **된다.** cmake 구성→빌드→설치 무경고. 라이브러리 3.4 MB, 외부 의존은 libstdc++/libm/libgcc/libc뿐 (충돌 요소 없음) |
| 전이형 초기상태를 프로그램으로 지정 가능한가 | **된다.** 190 m/s·상승각 32°(계약 1이 거부하는 바로 그 상태)를 `FGInitialCondition`으로 직접 지정 |
| PVA를 로컬 미터 프레임으로 뽑을 수 있는가 | **된다** (단서 아래) |
| 비용은 | **스텝당 6.5~9.2 µs.** 12초 궤적 = dt 0.01에서 11 ms |
| 적분 스텝 민감도 | dt 0.001→0.02에서 12초 후 고도차 0.3 m, 속력차 0.01 m/s |
| 재현성 | 동일 프로파일 2회 실행 **비트 단위 일치** (탐색의 전제) |

**부분 인계창 도달 — 표현을 정확히 한다:**

- 손으로 고른 입력 프로파일은 시험한 번들 예제 기체 두 종 **모두 반대
  방향**으로 갔다(감속하며 상승각 증가). 모델은 물리를 줄 뿐 목표에 데려다주지
  않는다는 것이 그대로 나타났다.
- 가장 조악한 대체물(elevator×throttle 2차원 격자, 27개 프로파일, 각 30초)
  을 **761 ms**에 돌렸고 **15개가 부분 인계창**에 1초 이상 머물렀다.
  최단은 t=1.34초 진입 → **t=2.33초 체류 충족**(두 시각은 다르다),
  그때 \|v\|=171.1 m/s, γ=22.0°.
- ⚠ **이것은 "전이 탐색이 닫혔다"가 아니다.** 검사한 것은 속력과
  비행경로각 **둘뿐**이고, 실제 인계에 필요한 나머지는 하나도 보지 않았다:
  `pvaEnvelopeProblem()` 통과, 전역 경로 진입점과의 위치 정합
  (`route_start_s`), 경로 접선과 속도 방향 정렬, 가속도·하중 조건,
  인계 영역 체류 판정, 지형·구역 검사.
  정확한 서술은 **"예제 모델에서 2차원 상태창 진입 후보를 계산 가능한
  시간에 찾았다"**이다. 인계 성공 선언은 위 항목이 붙은 뒤에만 한다.

**좌표 추출 — 고정 ECEF→ENU 프레임으로 확정 (2026-08-07 2차 수정):**

초판 프로브는 위치를 초기점 고정 평면근사로, 속도·가속도를 기체 현재
위치의 local NED(매 순간 회전)로 뽑았다 — **프레임 혼용**이라 삼중항이
자기 일관될 수 없었다("타원체 변환" 문제로 처음 오진). 확정 방식:

```
R0     = t=0의 ECEF→NED (JSBSim Tec2l, 1회 캡처 후 고정)
p_enu  = ned2enu( R0 · (r_ecef − r_ecef(0)) )    ← GetLocation()
v_enu  = ned2enu( R0 · v_ecef )                   ← GetECEFVelocity()
a_ecef = Ti2ec·UVWidot − 2ω×v − ω×(ω×r)          (해석적, 채택)
```

- 검증: `|d(p)/dt − v|` = 속력의 0.010%, **dt와 함께 O(dt²) 수렴**;
  `|d(v)/dt − a|`도 명령 계단의 액추에이터 응답 구간([0.95, 1.40 s])을
  제외하면 dt와 함께 수렴(0.036→0.0015 m/s²). 남는 비수렴 잔차는 위치가
  t=1.0⁺에 고정되는 **물리적 불연속**(계단 명령)이다.
- `Tb2l·UVWdot`(수송항 없음)은 53.3% 어긋난다 — 쓰면 안 된다.
  `Tb2l·(UVWdot+PQR×UVW)`는 이 짧은 호에서 해석식과 수치상 같았지만
  이동 프레임이므로 원리상 긴 호에서 어긋난다 — 고정 프레임 해석식을
  채택한다.

**PVA → 5차 Hermite 재표현 — 프레임 수정 후의 유효한 측정:**

| 항목 | 결과 |
|---|---|
| knot P/V/A 일치 | 1e-14 (구성상 정확) |
| 접합 C⁰/C¹/C² | ≤ 1e-13 |
| 구간 내부 오차 | **P/V/A 전부 knot 세분화와 함께 수렴**: 0.1 s knot에서 0.4 mm / 0.002 m/s / 0.13 m/s², 2 s에서 0.40 m / 0.74 m/s / 4.75 m/s² |
| jerk | 피크 35.1 m/s³, 접합 점프 34.6 m/s³ — 명령 계단 위치에 국한. §8(c)에 따라 측정 전용이며, 실행 계층 허용치 확보 시 게이트로 승격 |

- 초판에서 "속도 오차 1.03 m/s가 knot과 무관, 가속도 오차는 촘촘할수록
  악화, jerk 529 m/s³"로 보였던 것은 **전부 프레임 혼용의 산물**이었다.
  다항식 고유 성질이라는 초판 해석은 철회한다.
- ⇒ 완료 선언의 정확한 범위: **"JSBSim 예제 모델에서 고정 ENU PVA를
  조각별 5차 다항식으로 재표현하고 C² 연결할 수 있으며, 근사 오차가 knot
  간격에 따라 수렴함을 확인했다"** — 표현 계층 검증까지다. 실제 전환
  인계·실행 가능성·적절한 knot 간격은 이후 §13 구현이 담당했고, jerk는
  §8(c) 결론대로 측정 전용(실행 계층 허용치 확보 시 게이트 승격)이다.
  실행 통합의 관건은 어댑터가 아니라 입력의 프레임 일관성 — 위 추출식을
  계약으로 삼는다.

**그 밖의 통합 비용:**

1. **6DOF를 다 쓰지는 않는다.** 후단 계약이 PVA·C²라 병진 성분만 소비한다.
   자세·각속도는 실행 가능성 판정에 쓰되 궤적 표현에는 넣지 않는다
   (§11.5의 3DOF 우선 방침과 일치).
2. **컨테이너 반영**은 **뒤로 미룬다.** 개발용 Dockerfile 편입보다 어댑터
   정합성(위 프레임 변환) 해결이 먼저다. apt 후보가 없어 소스 빌드이며,
   실험은 `run.sh`가 자체적으로 받아 빌드한다.

**재현 방법**: `experiments/jsbsim_probe/` (추적됨). `./run.sh`가 고정 버전
(v1.3.1)을 받아 빌드하고 RPATH를 박아 실행한다 — `LD_LIBRARY_PATH` 불필요.
설치·빌드 경로는 **버전별 분리**라 `--version v1.2.1` 재검증이 실제 그
버전의 라이브러리로 돈다(공용 경로였던 초판은 신버전 라이브러리에 구버전
데이터가 섞일 수 있었다). 태그와 **고정 SHA**를 대조해 불일치 시 실패한다.
RPATH는 `$ORIGIN` 상대라 **컨테이너(/ws)와 호스트(/home/...) 어느 경로에서
실행해도** 라이브러리를 찾는다(절대 RPATH였던 초판은 호스트에서 실패).
root 소유 캐시는 감지해 소유권 수정/재생성 안내를 내고, git dubious
ownership은 `safe.directory`로 처리한다. cmake 없는 호스트에서도 warm
캐시가 있으면 재실행된다. 컨테이너·호스트 양쪽에서 실행 확인 완료.

**PVA 인터페이스 프로브 (2026-08-07, 스펙 없이 가능한 마지막 선행 작업):**

dwell 완료 PVA를 플래너 경계 API에 실제로 통과시켰다(하니스 변형
`pvaprobe`, `MMP_PVA_CSV`로 다른 상태 주입 가능). 확인한 것은 인터페이스
계약뿐이다:

- 고정 ENU 변위 → 플래너 좌표: **mission anchor 합성으로 절대 고도 보존**
  (anchor z 15 u = JSBSim 초기 ASL 1500 m — 공기밀도 판정이 의미를 가짐)
- SI → 플래너 단위 변환 (÷100)
- `pvaEnvelopeProblem()` 호출, 그리고 **판정·사유가 PlanResult
  reason/detail까지 원문 그대로 전파**되는 것 (plan()과 validator의 일치)

결과: 부분 인계창(속력 171 m/s·γ 22°)을 통과한 그 상태가 전체 PVA
판정에서는 **뱅크 한계의 277% 요구로 거부**된다(순 하강 가속 −30.7 m/s²).
"부분 상태창 도달은 인계가 아니다"가 코드로 확인된 것이며, 예제 기체와
순항 모델이 다른 기체이므로 이 거부는 물리 검증이 아니다 — 예측된 정상
결과다. 이로써 **스펙 없이 가능한 계약 2 선행 작업은 전부 마감**됐다.

**남은 전제는 그대로다**: 위 결과는 **예제 기체**로 얻은 **구조 데모**다.
JSBSim 프로젝트 자신이 번들 기체를 공개 자료 기반의 근사 모델(교육·오락
목적)이라고 명시한다. §11.3의 물리 타당성 층은 여전히 비어 있고, 실제
플랫폼 적합성은 아래 데이터가 들어온 뒤에만 판정할 수 있다.

**공개 모델을 써도 여전히 필요한 기체 정보** (§8과 같은 대기 목록):
질량과 필요시 질량 변화, 추력/추진 모델, 공력 계수 또는 테이블,
6DOF면 관성모멘트, 허용 상태·제어 범위, 순항 모델과의 인계 조건.
이 데이터 없이는 예제 기체로 **구조 데모**는 되지만 실제 플랫폼에 대해
"실행 가능하다"고 주장할 수 없다 — §11.3의 물리 타당성 층이 비어 있는
것과 같은 이야기다.

### 11.4 구조적 선례와 그 한계

- `TerminalPhase`(prescribed 나선 하강)가 구조 템플릿: 옵티마이저 없이
  기하를 생성해 같은 `poly_traj::Trajectory`에 5차 Hermite 조각으로 넣어
  이음매가 C²가 되게 한다.
- 정정(적대적 검증): TerminalPhase가 "감사되지 않는다"는 서술은 틀렸다.
  `logFinalEvaluation`이 스티치된 비행을 0.1초 간격으로 샘플링하며
  `"terminal"`도 등록된 phase로 판정된다. 다만 그 판정은 **순항 모델**로
  이뤄지므로, 전이 구간에는 §4의 단계별 평가기가 반드시 필요하다.

## 12. 진입 후보 탐색 — 순차 판정 파이프라인

```
경로 후보 생성 (기존 calm/κ/grade/zone 선별 그대로)
   ↓
순항 진입 적합성 (기존 규칙 + pvaEnvelopeProblem)
   ↓
전환 모델 도달 가능성 (전환 한계로 판정 — 새로 추가되는 단계)
   ↓
가능한 후보 중 선정
   ↓ 전부 실패
TRANSITION_GENERATION_FAILED   (direct fallback 금지)
```

구현 지침(조사로 확정):

- `authorContractsFromRoute` 안의 선별 블록을 **값 반환 메서드로 추출**한다
  (`screenHandoffCandidates(...) -> PhaseScreen`). 현재는 const 메서드가
  mutable 멤버(`dep_candidates_`, `arr_candidates_`, `phase_tan_grade_`,
  `phase_turn_radius_u_`)에 쓰는 블랙보드 구조라, 다른 한계를 쓰는 세 번째
  소비자(전환)가 붙으면 순항 선별과 **교차 오염**된다. 추출은 미관이 아니라
  안전 요건이다.
- 하한 공식은 **주입식 정책**으로 바꾼다: `depRequired`와
  `phase_turn_radius_u_`(1.5 마진·`load_factor_max` 내장)를 `ReachLimits`
  구조체 필드로. 순항 선별은 순항 수치를, 전환 선별은 §8 수치를 넘긴다.
  전환 선별은 여기에 **속도 변화 항**이 추가로 필요하다 — 현재 하한은
  \|v0\| 일정을 가정하는데, 전이는 정확히 그 가정을 깨뜨린다.
- `validateConnector`는 전환 콘을 넘겨주면 재사용 가능(지형·존·grade 기하
  검사). 단 이것은 **기하 스크린일 뿐** 모델 판정이 아니다.
- **존 정책 상속 (정정 기록)**: 현행 `evaluateFlight`의 위험도는 **통계일
  뿐 CLEAN 판정에 들어가지 않는다** — 불가피 판정된 소프트 존 통과가
  허용되는 설계라 그 자체로 오류는 아니다. 그러나 전이 생성기는 전역
  3-pass가 확정한 존 정책을 **별도 합격 게이트로 상속**해야 한다:
  하드 유지(비면제) 존 접촉 = 후보 실격; 전역이 불가피로 선택한 소프트
  존은 통과 가능하되 노출을 측정·최소화; 단순 risk_max 기록 후 승인은
  금지.
- `buildDepartureConnector` / `contractFromVertex`는 전이 생성기로 재사용
  **금지**. 시간이 없고, 종료 가속도를 0으로 못박으며(`contractFromVertex`는
  `c.t = 0`, `c.acc = Zero`), 방향이 경로 정점 현으로 고정된다.
- `solveGated`의 `lastEnvPeak() > audit_envelope_peak_max` 게이트는 **순항
  모델 감사**이므로 전이 구간을 판정해서는 안 된다. 진입점 이후 첫 체인
  구간에는 그대로 둔다.
- 슬라이스는 `route_start_s`를 체인 내부로 흘리지 말고, `cutAtArc(route,
  cap, s)`로 **부분 경로를 한 번 실체화**해 기존
  `authorContractsFromRoute`/`sliceCommittedRoute`에 넘긴다. 그러면 호장
  원점·`route.front().z()` 기준·가상 head cut이 전부 수정 없이 동작한다.

기존 코드에서 함께 드러난 결함 — **스펙 독립 정리분은 2026-08-06 처리 완료**:

| 결함 | 상태 |
|---|---|
| departure 재시도에서 `edge_cap` 미대입 → 커넥터 solve가 스칼라 고도 밴드로 판정 | **수정** — `capAlongRoute()`가 최근접 경로 구간의 cap을 샘플링(절단 지점과 같은 "천장을 좁히지 않는다" 규칙) |
| N==2에서 `arr_candidates_`가 비어 arrival 구제 불가 | **정책 명문화** — 공유 핸드오프는 양방향 선별을 함께 통과한 지점이라 arrival만 옮기면 departure 선별이 무효화된다. 재시도 없음을 명시하고 direct로 강등, 사유에 N=2를 남김 (회귀 `arredge2`) |
| 계획 상태 누출: `segments_`가 계획 중 감소, phase 후보가 mutable 멤버 | **수정** — `resetPlanState()`가 plan() 첫 문장, `segments_requested_`가 유일한 계획 간 값 (회귀 `twophase`) |
| 후보 상한 3 리터럴 | **파라미터화** — `chain/phase/max_candidates`. 점수 기준은 도달성 모델이 없어 보류(선착순 유지) |
| 스티치 판정을 버림 (direct fallback만 게이트) | **수정** — `evaluateFlight()`가 판정 권위, `stitchedVerdictResult()`가 2단 정책: 지형 관통·순항 미도달 → FAILED(`STITCHED_FLIGHT_UNSAFE`), 영역 초과 → DEGRADED |
| arrival 선별에 `depRequired` 상당 하한 없음(도달성 선별은 departure 전용) | **보류** — 도달성 일반화는 전환 모델 스펙 이후 (§12 파이프라인) |

## 13. 오케스트레이션 소유권

- **FSM**: 계획 시점, 시작 PVA 해석과 AGL 변환, SI→단위 변환, 시퀀스·롤백,
  `PlanResult` 소비와 발행. **영역 분류·경계 검증·강등 판정은 소유하지
  않는다.** 현재 `replan_fsm.cpp`의 단일 경로 블록이 `plan()`의 정책을
  복제하고 있어, 분류가 세 상태로 늘면 정책이 두 벌로 갈라진다 — 계약 2
  착수 시 단일 진입으로 합친다.
- **SegmentChainPlanner(또는 그 자리의 조정 계층)**: 단계 순서, 분류 분기,
  경로 커밋, 진입점 선정, 슬라이싱, head BC 인계, 스티칭, `pm_->traj_`
  저장, 시각화, `PlanResult` 작성. 전이 기하와 한계 수치는 소유하지 않는다.
- **TransitionPhase**: 생성·다항화·§6 내부 검사. `PathManager`도 노드
  파라미터도 로깅도 건드리지 않는 순수 컴포넌트(`TerminalPhase` 형태:
  정적 함수, 지형은 `std::function` 주입, 한계는 구조체 인자,
  `poly_traj::Trajectory` 반환).
- **PathManager**: 변경 없음. 임의 호장 절단은 `sliceCommittedRoute`가 이미
  쓰는 `Cut{seg, u}` 기계와 같아 새 seam이 필요 없다.

착수 전 선행 정리(전이 코드보다 먼저):

1. `planRouteParallel`을 `commitRoute()` + `planOverRoute(route, cap,
   head_pva, tail, parallel)`로 분리. 그래야 계약 2가 "분류 → 커밋 → 전이 →
   절단 → 체인 → 앞에 붙이기"의 읽히는 순서가 된다.
2. `logFinalEvaluation`에서 판정을 `evaluateFlight()`로 분리(로깅과 판정
   분리, §4). 분리 전에 전이를 끼우면 전이 구간이 순항 영역으로 판정되어
   계약 2가 막으려는 바로 그 실패가 재현된다.
3. `fallback()` 람다의 호출 지점 전부를 전이 활성 시 FAILED로. 이 람다는
   **미션 시작점부터** 단일 계획을 세우므로, 전이가 필요한 미션에서 이게
   돌면 순항 영역 밖 상태에서 순항 모델 계획이 조용히 날아간다 — 문서가
   기원 결함으로 인용하는 548.7 u 벌레 그 자체.
4. 계획 범위 상태 정리: `segments_`가 계획 도중 재작성되고
   (`segments_ -= 1`), phase 후보들이 mutable 멤버다. 진입 후보를 바꿔가며
   재진입하는 순간 낡은 N과 낡은 후보 목록을 물려받는다.
5. `[VEL-ALIGN]`/`[STALL-FLOOR]`는 전이 종료 상태에 **적용 금지**(처방된
   궤적 유래 상태). route 모드에 복제된 판본이 있으므로 양쪽 다 확인.
