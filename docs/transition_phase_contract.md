...
- ⇒ **어댑터 실험은 예제 기체 기준 완료**: 자기 일관된 PVA가 주어지면
  Hermite 재표현 오차는 knot 간격으로 제어된다. 실전 통합의 관건은
  어댑터가 아니라 입력의 프레임 일관성이다 — 위 추출식을 계약으로 삼는다.
...
데이터가 섞일 수 있었다). 실행 시 태그와 **commit SHA**를 함께 출력한다.
