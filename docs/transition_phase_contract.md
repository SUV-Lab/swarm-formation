...
**로컬 프레임 가속도 — 어떤 식이 맞는지 실측으로 확정:**

`FGAccelerations::CalculateUVWdot`은 `vUVWdot`을 **동체 성분의 변화율**로
계산한다(수송항 `(vPQR + 2·Ti2b·ω_planet) × vUVW`를 이미 뺀 값). 따라서
`Tb2l * UVWdot`은 d(V_local)/dt가 **아니다**:
...
수송항을 되돌린 쪽을 쓴다. 수치미분은 교차검증용으로만 남긴다.

**PVA → 5차 Hermite 재표현 (§6 검증 항목 실측)**

| 항목 | 결과 |
|---|---|
| knot P/V/A 일치 | 1e-14 이하 (구성상 정확) |
| 조각 간 C⁰/C¹/C² | 0 / 7e-15 / 1e-13 |
| 구간 내부 위치 오차 | 0.1 s knot에서 8 mm → 2 s에서 0.39 m (수렴) |
| 구간 내부 속도 오차 | **knot 간격과 무관하게 ~1.03 m/s** |
| 구간 내부 가속도 오차 | 촘촘할수록 **악화** (0.1 s에서 31.9, 1 s에서 3.2 m/s²) |
| jerk | 피크 529 m/s³, 접합 점프 35.6 m/s³ — 측정만, 상한은 §8 대기 |

**속도 오차가 수렴하지 않는 원인이 이 실험의 핵심 발견이다.** 소스 삼중항이
스스로 어긋나 있다: `|d(p)/dt − v| = 0.554 m/s`(속력의 0.29%). 위치와
속도를 **양 끝에서 동시에** 고정한 5차식은 구간 안에서 그만큼의 여분 이동을
흡수해야 하므로 중간 속도가 약 2ε 초과한다 — 구간 길이와 무관하게.
실측 1.03 ≈ 2 × 0.554로 예측과 일치.

⇒ **프레임 변환은 각주가 아니라 어댑터의 지배적 오차원이다.** 해결은 knot을
촘촘히 하는 게 아니라 상류의 **타원체 국소접평면 변환**이다(현 프로브는
지심위도+구면 반지름 근사). 가속도 오차가 촘촘할수록 커지는 것은 별개의
다항식 성질로, §6이 요구하는 **구간 내부 극값 검사**가 왜 필요한지를 보여
주는 실측 사례다(jerk 피크 529 m/s³).
...
v1.2.1도 동일하게 동작하며 `--version`으로 되돌려 확인 가능.
