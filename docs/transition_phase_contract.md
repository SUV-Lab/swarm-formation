...
- 가장 조악한 대체물(elevator×throttle 2차원 격자, 27개 프로파일, 각 30초)
  을 **801 ms**에 돌렸고 **15개가 순항 진입 영역**
  (\|v\| 132~230 m/s, \|γ\| ≤ 30°)에 도달했다. 최단 진입 1.34초,
  그때 상태 \|v\|=171.1 m/s, γ=22.0°.
- 그 시점 PVA가 그대로 체인의 prescribed head 형태로 나온다:
  p=(−0.1, −366.5, 1705.8) m, v=(−0.0, −158.6, 64.1) m/s,
  a=(−0.02, −2.95, −30.67) m/s².

⇒ 구조는 닫힌다. 비용도 문제가 아니다(수백 개 후보 탐색이 초 단위).

**단서 — 통합 시 실제 비용이 되는 것:**

1. **가속도는 로컬 프레임으로 직접 안 나온다.** JSBSim이 노출하는
   `accelerations/udot|vdot|wdot`는 **동체축**이다. Hermite/MINCO는 p·v·a가
   같은 프레임이어야 하므로 로컬 속도의 수치미분이 필요하다 — 위 측정도
   중앙차분으로 얻었다. 이 미분 오차가 §6의 접합 jerk 감사 정밀도의 하한을
   정한다.
2. **좌표계·단위 변환.** JSBSim은 피트·측지좌표, 우리는 로컬 미터
   (1 u = 100 m). 전이 규모(수 km)에서는 평면근사 오차가 지형 해상도
   (30~60 m)보다 훨씬 작아 문제없지만, 변환은 명시적으로 한 곳에서 해야
   한다.
3. **6DOF를 다 쓰지는 않는다.** 우리 후단 계약은 PVA·C²라 병진 성분만
   소비한다. 자세·각속도는 실행 가능성 판정에는 쓰되 궤적 표현에는 넣지
   않는다(§11.5의 3DOF 우선 방침과 일치).
4. **컨테이너 반영**: Dockerfile에 JSBSim 소스 빌드 또는 패키지 추가 필요
   (apt 후보 없음). 항공기 데이터 4 MB + 엔진/시스템 0.7 MB 동봉.

**남은 전제는 그대로다**: 위 결과는 **예제 기체**로 얻은 **구조 데모**다.
§11.3의 물리 타당성 층은 여전히 비어 있고, 실제 플랫폼 적합성은 아래
데이터가 들어온 뒤에만 판정할 수 있다.
