...
- 상태: **설계 확정 / 참조 모델 재선정 대기** (2026-08-07). 방침은 확정 —
  ...
  외부 플랫폼 스펙은 기다리지 않는다. 단 첫 모델 선택(ADR-0001, F-16A)은
  리뷰에서 기각·보류됐다(단계 의미 불일치, 군사용어 방침 충돌, 데이터
...
| 상태·제어·물리 채널 구성 | **ADR-0001로 확정** — JSBSim F-16A (v1.3.1 고정 SHA)의 데이터 파일이 곧 채널 정의 |
| 질량 변화, 마하 의존 항력, 추가 제어 채널(TVC 등) | 참조 기체에 있는 것만 사용 (F100 연료 소모 모델 있음, TVC 없음, M<0.81에서 파동항력 0) |
...
- 손으로 고른 입력 프로파일은 f16·X15 **둘 다 반대 방향**으로 갔다
  (감속하며 상승각 32°→50°/68°). 모델은 물리를 줄 뿐 목표에 데려다주지
  않는다는 것이 그대로 나타났다.
