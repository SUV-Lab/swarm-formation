...
- 상태: **구현 준비 — 스펙은 ADR-0001로 확정** (2026-08-07). 목표를 실제
  플랫폼 인증이 아니라 **공개 참조 기체(JSBSim F-16A) 기반 기능 구현·연구
  실험**으로 확정하면서, §8 수치와 §11 모델 결정이
  `docs/adr/0001-reference-vehicle.md`로 동결됐다. 외부 스펙 대기는 해제.
  모든 산출물은 "공개 참조 모델 기반 구조 검증"으로 표기하며 실제 플랫폼
  적합성은 범위 밖.
...
## 8. 전환 한계 수치 — **ADR-0001로 동결** (연구 정책 v0)
...
v0 수치는 ADR-0001의 "전환→순항 겹침 영역과 완료 조건" 표가 정의한다:
전환 속력 60–275 m/s, 상승각 ±60°, 하중 ≤ 4 g, jerk 상한은 1단계 구현의
측정 분포로 설정, 완료 = 겹침 영역(속력 [132, 230] ∧ |γ|≤30° ∧
pvaEnvelopeProblem 통과 ∧ 접선 정렬 ≤10°) 체류 ≥ 1.0 s. **연구 정책이며
실험으로 조정한다** — 실제 플랫폼 값 주장이 아니다.
