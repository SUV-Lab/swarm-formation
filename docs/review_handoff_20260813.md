...
- `transwp`에 전단 실패 주입 팔 추가 — 이유가 바뀌지 않아야 한다
...
- **leg 검색 실패 자체.** 이 하네스에서 FM2 전단을 실패시킬 방법이 없다 [재현]:
  맵 밖 웨이포인트는 클램프되어 goal에 도달하고(leg goal로도 leg start로도 확인),
  빈 SDF에서는 `obstacle_clearance`가 무시되며, 테스트에 노출된 장애물 삽입 API가 없다.
