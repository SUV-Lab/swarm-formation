...
**8차 검토 3건 — 전부 사실이었다**:
1. **`trajectory_id`가 단조가 아니었다.** `setGlobalTraj`가 0으로 되돌리고
   `setLocalTraj`가 증가시켜 **모든 정상 궤적이 id 1**이었다. 그러면 이전 비행의 abort가
   새 비행을 멈춘다 — id가 막으려던 바로 그 혼동이다. 내 `exec_abort_test`는 7·8을 직접
   만들어 **생산 ID 경로를 우회**했으므로 이걸 볼 수 없었다. 리셋되지 않는
   `next_traj_id_`로 바꾸고, **실제 계획 두 번**으로 `id2 > id1`을 고정했다
   (변이: 리셋 복원 → `1 -> 1`로 사망)
2. **`WAIT_EXTERNAL_RECOVERY`가 덫이었다.** 주석은 새 미션이 빠져나가게 한다고 했지만
   전환표에 그 분기가 없었다. 추가하고 문서에 적었다
3. **지형 차단도 `OBSTACLE_BLOCKED`로 발행**됐다. 재검사가 reason을 하드코딩하고 있었다.
   호출자가 주는 것으로 바꿨다 (변이: 다시 하드코딩 → 사망)
